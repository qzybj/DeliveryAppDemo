////////////////
// app_routing.js
//虽然通过middleware方式，把“/”和“/about”独立出来，但和优雅的高可读代码还没半毛钱关系。
// Routing是基于Middleware特性，它其实就是匹配不同的request到不同的handler上
////////////////
var MockDB = function () {
    var getTargetById = function (list, id) {
    ...
    var getOrderDetail = function (orderData,orderId) {
        var target = getTargetById(orderData, orderId);
...
app.get("/api/login", function (req, res) {
    ...
    res.end(res.json({success: true, data: {authenticationToken: "abc01234567890defgh"}}));
});
...
app.get("/api/orders", function (req, res) {
    console.log(req.toString());
    var returnMockDB = new MockDB();
    res.json({success: true, data: returnMockDB.orderData});
});
app.get('/api/detail/:orderId', function (req, res) {
    var orderId = req.params.orderId;
    if(orderId!=null&&orderId!=undefined){
        console.log(req+orderId);
        var returnMockDB = new MockDB();
        res.json({success: true, data: returnMockDB.getOrderDetail(returnMockDB.orderData,orderId)});
    }else{
        res.end('orderId is '+orderId);
    }
});
